...
BOOL DPropD3D::OnInitDialog() 
{
	...
	_itoa(IDC_TEXT_DXLEVEL,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,"",value);
	if (value != "") GetDlgItem(IDC_TEXT_D3D)->SetWindowText(value);
...
void DPropD3D::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_DXLEVEL, m_dxLevelC);
...
BEGIN_MESSAGE_MAP(DPropD3D, CPropertyPage)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_DXLEVEL, OnSelchangeComboDxLevel)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void DPropD3D::OnSelchangeComboDxLevel() 
{
	// TODO: Add your control notification handler code here
	int nIndex = m_dxLevelC.GetCurSel();
	if (nIndex == CB_ERR)
		return;

	switch (nIndex) {
		case 1: m_dxLevel=3; break;
		case 2: m_dxLevel=5; break;
		case 3: m_dxLevel=6; break;
		default :
			m_dxLevel=0; break;
...
void DPropGeneral::OnRegisterVrml() 
{
	...
	BOOL ret = m_ctrl->UpdateRegistry();
}
...
BOOL DAskHardware::OnInitDialog() 
{
	...
	GetDlgItem(IDC_ASK1)->GetWindowText(defaultValue);
	_itoa(IDC_ASK1,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_ASK1)->SetWindowText(value);

	GetDlgItem(IDC_ASK2)->GetWindowText(defaultValue);
	_itoa(IDC_ASK2,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,"",value);
	if (value != "") GetDlgItem(IDC_ASK2)->SetWindowText(value);

	GetDlgItem(IDC_ASK3)->GetWindowText(defaultValue);
...
DPropCache::DPropCache(CGLViewCtrlCtrl *ctrl) : m_ctrl(ctrl), CPropertyPage(DPropCache::IDD)
{
	...
	m_useCache = FALSE;
	m_useMediaCache = FALSE;
	m_cacheDirectory = _T("");
	m_mediaCacheDirectory = _T("");
...
BOOL DPropCache::OnInitDialog() 
{
	...
	_itoa(IDC_MEDIA_CACHE,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,"",value);
	if (value != "") GetDlgItem(IDC_MEDIA_CACHE)->SetWindowText(value); // Laurent - Jan.2000
...
//	DDX_Check(pDX, IDC_CACHE_READ, m_cacheRead);
	...
	DDX_Check(pDX, IDC_MEDIA_CACHE, m_useMediaCache); // Laurent - Jan. 2000
	DDX_Text(pDX, IDC_DIRECTORY, m_cacheDirectory);
	DDX_Text(pDX, IDC_MEDIA_DIRECTORY, m_mediaCacheDirectory); // Laurent - Jan. 2000
...
BOOL DPropD3D2::OnInitDialog() 
{
	...
	GetDlgItem(IDC_PIII)->GetWindowText(defaultValue);
	_itoa(IDC_PIII,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_PIII)->SetWindowText(value);

	GetDlgItem(IDC_SOFTWARE_LIGHTING)->GetWindowText(defaultValue);
	_itoa(IDC_SOFTWARE_LIGHTING,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_SOFTWARE_LIGHTING)->SetWindowText(value);

	GetDlgItem(IDC_USE_D3D_CULLING)->GetWindowText(defaultValue);
	_itoa(IDC_USE_D3D_CULLING,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_USE_D3D_CULLING)->SetWindowText(value);

	
	GetDlgItem(IDC_T_NURBS_SETTINGS)->GetWindowText(defaultValue);
	_itoa(IDC_T_NURBS_SETTINGS,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_T_NURBS_SETTINGS)->SetWindowText(value);

	GetDlgItem(IDC_T_TARGET_FPS)->GetWindowText(defaultValue);
	_itoa(IDC_T_TARGET_FPS,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_T_TARGET_FPS)->SetWindowText(value);

	GetDlgItem(IDC_T_TESS_FACTOR)->GetWindowText(defaultValue);
	_itoa(IDC_T_TESS_FACTOR,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_T_TESS_FACTOR)->SetWindowText(value);

	GetDlgItem(IDC_T_NURBS_MODE)->GetWindowText(defaultValue);
	_itoa(IDC_T_NURBS_MODE,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_T_NURBS_MODE)->SetWindowText(value);
...
/////////////////////////////////////////////////////////////////////////////
// DAskHardwareProblem dialog


DAskHardwareProblem::DAskHardwareProblem(CGLViewCtrlCtrl *ctrl, CWnd* pParent /*=NULL*/) : 
	m_ctrl(ctrl), 
	CDialog(DAskHardwareProblem::IDD, pParent)
{
	//{{AFX_DATA_INIT(DAskHardwareProblem)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}

BOOL DAskHardwareProblem::OnInitDialog() 
{
	CString value, defaultValue;
	char buf[60];
	buf[0]='p';

	GetDlgItem(IDC_TEXT_ASK_HW_PROB)->GetWindowText(defaultValue);
	_itoa(IDC_TEXT_ASK_HW_PROB,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_TEXT_ASK_HW_PROB)->SetWindowText(value);

	GetDlgItem(ID_SAFE_MODE)->GetWindowText(defaultValue);
	_itoa(ID_SAFE_MODE,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(ID_SAFE_MODE)->SetWindowText(value);

	GetDlgItem(ID_NORMAL_MODE)->GetWindowText(defaultValue);
	_itoa(ID_NORMAL_MODE,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(ID_NORMAL_MODE)->SetWindowText(value);

	return CDialog::OnInitDialog();
}

void DAskHardwareProblem::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(DAskHardwareProblem)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(DAskHardwareProblem, CDialog)
	//{{AFX_MSG_MAP(DAskHardwareProblem)
	ON_BN_CLICKED(ID_NORMAL_MODE, OnNormalMode)
	ON_BN_CLICKED(ID_SAFE_MODE, OnSafeMode)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// DAskHardwareProblem message handlers

void DAskHardwareProblem::OnNormalMode() 
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);

	//CDialog::OnOK();	
	EndDialog(ID_NORMAL_MODE);

}

void DAskHardwareProblem::OnSafeMode() 
{
	// TODO: Add your control notification handler code here

	UpdateData(TRUE);
	//CDialog::OnCancel();	
	EndDialog(ID_SAFE_MODE);
	
}

/////////////////////////////////////////////////////////////////////////////
// DPropPerformance property page

IMPLEMENT_DYNCREATE(DPropPerformance, CPropertyPage)

DPropPerformance::DPropPerformance(CGLViewCtrlCtrl *ctrl) : 
	m_ctrl(ctrl), 
	CPropertyPage(DPropPerformance::IDD)
{
	m_textureSizeIndex =0;
	//{{AFX_DATA_INIT(DPropPerformance)
	m_useMovies = TRUE;
	m_useSounds = TRUE;
	m_useTextures = TRUE;
	m_visbilityFactor = 1.0f;
	m_info = _T("");
	//}}AFX_DATA_INIT
}

DPropPerformance::~DPropPerformance()
{
}



BOOL DPropPerformance::OnInitDialog() 
{
	CString value, defaultValue;
	char buf[60];
	buf[0]='p';

	GetDlgItem(IDC_USE_MOVIES)->GetWindowText(defaultValue);
	_itoa(IDC_USE_MOVIES,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_USE_MOVIES)->SetWindowText(value);

	GetDlgItem(IDC_USE_SOUNDS)->GetWindowText(defaultValue);
	_itoa(IDC_USE_SOUNDS,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_USE_SOUNDS)->SetWindowText(value);

	GetDlgItem(IDC_USE_TEXTURES)->GetWindowText(defaultValue);
	_itoa(IDC_USE_TEXTURES,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_USE_TEXTURES)->SetWindowText(value);

	GetDlgItem(IDC_T_DETAILS)->GetWindowText(defaultValue);
	_itoa(IDC_T_DETAILS,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_T_DETAILS)->SetWindowText(value);

	GetDlgItem(IDC_T_VISIBILITY)->GetWindowText(defaultValue);
	_itoa(IDC_T_VISIBILITY,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_T_VISIBILITY)->SetWindowText(value);

	GetDlgItem(IDC_TEXT_MAX_TEXTURE_SIZE)->GetWindowText(defaultValue);
	_itoa(IDC_TEXT_MAX_TEXTURE_SIZE,&buf[1],10);
	m_ctrl->m_translator.Translate(buf,defaultValue,value);
	if (value != "") GetDlgItem(IDC_TEXT_MAX_TEXTURE_SIZE)->SetWindowText(value);
...
void DPropPerformance::DoDataExchange(CDataExchange* pDX)
{
	CPropertyPage::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(DPropPerformance)
	DDX_Control(pDX, IDC_TEXTURE_SIZE, m_textureSizeC);
	DDX_Check(pDX, IDC_USE_MOVIES, m_useMovies);
	DDX_Check(pDX, IDC_USE_SOUNDS, m_useSounds);
	DDX_Check(pDX, IDC_USE_TEXTURES, m_useTextures);
	DDX_Text(pDX, IDC_VISIBILITY_FACTOR, m_visbilityFactor);
	DDX_Text(pDX, IDC_INFO, m_info);
	//}}AFX_DATA_MAP

	if (!pDX->m_bSaveAndValidate) {
		m_textureSizeC.SetCurSel(m_textureSizeIndex);
	}
}


BEGIN_MESSAGE_MAP(DPropPerformance, CPropertyPage)
	//{{AFX_MSG_MAP(DPropPerformance)
	ON_CBN_SELCHANGE(IDC_TEXTURE_SIZE, OnSelchangeTextureSize)
	ON_NOTIFY(UDN_DELTAPOS, IDC_AUTO_LOD_SPIN, OnDeltaposAutoLodSpin)
	ON_NOTIFY(UDN_DELTAPOS, IDC_VISIBILITY_FACTOR_SPIN, OnDeltaposVisibilityFactorSpin)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// DPropPerformance message handlers

void DPropPerformance::OnSelchangeTextureSize() 
{
	int nIndex = m_textureSizeC.GetCurSel();
	if (nIndex == CB_ERR)
		return;
	m_textureSizeIndex = nIndex;
	
}

void DPropPerformance::OnDeltaposAutoLodSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;
...
void DPropPerformance::OnDeltaposVisibilityFactorSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_UPDOWN* pNMUpDown = (NM_UPDOWN*)pNMHDR;

	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
	m_visbilityFactor += -0.1* pNMUpDown->iDelta;

	if (m_visbilityFactor <0.01) m_visbilityFactor = 0.01f;
