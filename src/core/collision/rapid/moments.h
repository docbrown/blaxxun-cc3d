...
inline
void
mean_from_moment(float M[3], moment &m)
{
  M[0] = m.m[0];
  M[1] = m.m[1];
  M[2] = m.m[2];
...
inline
void
mean_from_accum(float M[3], accum &a)
{
  M[0] = a.m[0] / a.A;
  M[1] = a.m[1] / a.A;
  M[2] = a.m[2] / a.A;
...
inline
void
covariance_from_accum(float C[3][3], accum &a)
{
  int i,j;
  for(i=0; i<3; i++)
    for(j=0; j<3; j++)
...
inline
void
compute_moment(moment &M, const float p[3], const float q[3], const float r[3])
{
  float u[3], v[3], w[3];
