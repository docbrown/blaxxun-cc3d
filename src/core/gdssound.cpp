...
//	CWaitCursor				cursor; // HG 
    ...
    m_time = 0;
       ...
       if (DS3D_SUCCEED(result)) {
               ...
               dsbd.dwSize = sizeof(DSBUFFERDESC5); // TO DO : DX7 
...
GDSSoundEmitter::GDSSoundEmitter()
{
    ...
    InitializeCriticalSection(&m_CsDSB);

	TRACE("SDHM: Constructor: Setting m_pDSB to NULL ####################################\n");
    m_pDSB=NULL;
	m_lpCE=NULL;
	m_next = NULL;
	m_refCnt = 0;
...
GDSSoundEmitter::~GDSSoundEmitter()
{
    TRACE("SDHM: >>> GDSSoundEmitter<%08x>::~GDSSoundEmitter()\n", this);
	Release();

    TRACE("SDHM: Deleting Critical Section for m_pDSB.\n");
    DeleteCriticalSection(&m_CsDSB);
...
{
    TRACE("SDHM: >>> GDSSoundEmitter<%08x>::Release()\n", this);
    TRACE("SDHM: m_enabled<%d> \n", m_enabled);
	if (m_enabled) {
		Stop();
	}

    TRACE("SDHM: m_lpCE<%08x> \n ", m_lpCE);
	if(m_lpCE){
        TRACE("SDHM: calling m_lpCE->Release()\n");
		m_lpCE->Release();
        m_lpCE = NULL;
	} /* if */


    {   HCSlocker cs(&m_CsDSB);

        TRACE("SDHM: m_pDSB<%08x> \n", m_pDSB);
        if(m_pDSB) {
            TRACE("SDHM: calling m_lpDSB->Release()\n");
		    m_pDSB->Release();
            TRACE("SDHM: Setting m_pDSB to NULL ###########################################################\n!");
            m_pDSB = NULL;
	    } /* if */
    }
...
BOOL GDSSoundEmitter::Initialize(GSoundDevice *device,const char *fileName,BOOL spatialize)
{
    ...
    if (!spatialize || !DSdevice->m_has3d ||(DSdevice->m_quality <= GS_QUALITY_NO3D)) // NO 3D sound 
		m_pDSB = DSLoadSoundBuffer(DSdevice->m_lpDS, fileName,&m_BufferSize,m_result);
    else { 
		m_pDSB = DSLoad3DSoundBuffer(DSdevice->m_lpDS, fileName,&m_BufferSize,m_result);
		// above function will fall back to non 3d buffer automatically 
	}
    TRACE("SDHM: m_pDSB in Initialize[static] set to %08x ###############################\n", m_pDSB);

    if (m_result>0) {
		TRACE("Special return code %08x \n",m_result);
...
BOOL GDSSoundEmitter::Initialize(GSoundDevice *device, DWORD Frequency, DWORD Channels, DWORD BitsPerSample, double BufferLength, BOOL spatialize)
{
    ...
    {   HCSlocker cs(&m_CsDSB);
        ...
        TRACE("SDHM: m_pDSB set in Initialize[streaming] to %08x.#############################################\n", m_pDSB);
    }
...
BOOL GDSSoundEmitter::QueryInfo(BOOL &playing,float &secondsPlayed,float &totalSeconds, int &numLoops,float &level)
{
	...
	if (!SUCCEEDED(m_pDSB->GetStatus(&status))) 
		return FALSE;

	playing = (status & DSBSTATUS_PLAYING) != 0;
	if (m_enabled && !playing) {
		TRACE("DS Stopped emitter %d\n",status);
	}
	...
	if (m_muted && m_enabled ) { // it is in muted state !!!!!!!!!!!!!!
		if (m_loop)
			playing = TRUE;
		else {
			DWORD dt = (timeGetTime() - m_enabledTime); // stoped while muted ?
			if (dt < m_duration) {
				playing = TRUE;
				secondsPlayed = (float)dt / 1000.0f;
			}
			else { 
				playing = FALSE;
				//Stop(); // ???
...
DWORD GDSSoundEmitter::getSaveCursor()
{
  ...
  DS3D_SUCCEED(m_pDSB->GetCurrentPosition(&PlayCursor, &SaveCursor));
