...
BOOL CDIBSurface::Create(int cx, int cy,int ibitcount, CPalette* pPal)
{
    HPALETTE hPal = NULL;
    if (pPal) hPal = (HPALETTE)pPal->m_hObject;
    return Create(cx, cy,ibitcount, hPal);
}


BOOL CDIBSurface::Create(int cx, int cy,int ibitcount, HPALETTE hPal)
{
    if (!m_bUseCreateDIBSection && !m_bUseWinG) {
...
BOOL CDIBSurface::Load(const char* pszFileName/*=NULL*/)
{
    ...
    if (!Create(dib.GetWidth(), dib.GetHeight(),dib.GetBitsPerPixel(), &pal)) return FALSE;
