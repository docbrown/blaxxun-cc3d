...
GV_NODE_SOURCE_IMP(GvSpotLight,GvLight);

GvSpotLight::GvSpotLight() :
    beamWidth(1.570796),
	cutOffAngle(0.785398), // M_PI / 4.0;
    radius(100)

{
    GV_NODE_CONSTRUCTOR(GvSpotLight);
    isBuiltIn = TRUE;

	// from GvLight
    GV_NODE_ADD_FIELD(ambientIntensity);
    GV_NODE_ADD_FIELD(attenuation);
    GV_NODE_ADD_FIELD(beamWidth);
    GV_NODE_ADD_FIELD(color);
    GV_NODE_ADD_FIELD(cutOffAngle);
    ...
    GV_NODE_ADD_FIELD(dropOffRate); // ?? VRML 1.0
...
GvBool GvSpotLight::computeCopyOnInstance(GvCopyContext &context)
{
	setCopyOnInstance(TRUE);
	return GvNode::computeCopyOnInstance(context);
}

GvBool GvSpotLight::getBBoxEx(BBox &bbox)
{
