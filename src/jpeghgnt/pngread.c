...
int png_read(const char *file_name,image *im)
{
    ...
    */
//XX   png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, (void *)user_error_ptr, user_error_fn, user_warning_fn);
...
//xxx ??    info_ptr = png_create_info_struct();
   if (!info_ptr)
   {
      fclose(fp);
...
//XX    png_set_read_fn(png_ptr, (void *)user_io_ptr, user_read_fn);
