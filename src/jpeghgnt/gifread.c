...
static char * image_buffer;	/* holds jpeg output image */
static image *the_image;	/* output image */
static int row_offset=0;	/* current ouput scanline */
...
{
  int i,r,g,b;
  if (the_image ->cmap == NULL) image_alloc_cmap(the_image,cmaplen);

  for (i = 0; i < cmaplen; i++) {
    /* cmap[CM_RED][i]	 = */  r= (JSAMPLE) ReadByte(cinfo);
    /* cmap[CM_GREEN][i] = */  g= (JSAMPLE) ReadByte(cinfo);
    /* cmap[CM_BLUE][i]  = */  b= (JSAMPLE) ReadByte(cinfo);
    image_set_cmap(the_image,i,r,g,b);
  }
}
...
input_init (compress_info_ptr cinfo)
{
  ...
  the_image->fmt = IM_MAPPED8;
  the_image->error=image_alloc(the_image,the_image->fmt,cinfo->image_width,cinfo->image_height);
  the_image->backgroundIndex = (IBYTE) backgroundColor; 
  image_buffer = (char *) the_image->image;

  if (image_buffer == NULL )
     ERREXIT(cinfo->emethods, "out of memory allocting image");
...
#else
  register IBYTE *ptr;
  ptr = IROW(the_image,cur_row_number);
  for (col = cinfo->image_width; col > 0; col--) {
    c = LZWReadByte(cinfo);
    *ptr++ = c;
  }
...
get_interlaced_row (compress_info_ptr cinfo, JSAMPARRAY pixel_row)
{
  ...
  image_ptr = (*cinfo->emethods->access_big_sarray)	(interlaced_image, irow, FALSE);
...
/*
  read a gif image in a newly allocated image area
...
int gif_read(image *im,const char *jpegfile)
{
  ...
  /* that global image output structure */
  the_image = im;

  if ((cinfo.input_file = fopen(jpegfile, READ_BINARY)) == NULL) {
...
if (is_interlaced) {	 
   ...
   the_image->error=image_alloc(&tmpim,the_image->fmt,the_image->width,the_image->height);
   ...
   /*    (cinfo.methods->progress_monitor) (cinfo, row, cinfo->image_height); */
	...
	/* Figure out which row of interlaced image is needed, and access it. */
		...
		case 0:			/* first-pass row */
			...
			break;
		case 4:			/* second-pass row */
			irow = (row >> 3) + pass2_offset;
			break;
		case 2:			/* third-pass row */
		case 6:
			irow = (row >> 2) + pass3_offset;
			break;
		default:			/* fourth-pass row */
			irow = (row >> 1) + pass4_offset;
			break;
		}


	 memcpy(IROW(&tmpim,row), IROW(the_image,irow),cinfo.image_width); 

 }
	 // replace data in the_image 
	 free(the_image->image);
	 the_image->image = tmpim.image;
	 tmpim.image = NULL;
	 image_free_all(&tmpim);
  ...
  /* pass result parameter in image */
  image_buffer = NULL;
...
#endif
  if (cinfo.input_file != stdin) fclose(cinfo.input_file);
